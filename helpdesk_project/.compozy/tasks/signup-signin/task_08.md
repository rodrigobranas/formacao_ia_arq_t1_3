...
## Subtasks
- [x] 8.1 Create SignupPage component with form fields and validation
- [x] 8.2 Implement signup form submission with API call and error handling
- [x] 8.3 Create SigninPage component with form fields
- [x] 8.4 Implement signin form submission using AuthContext
- [x] 8.5 Add navigation links between signup and signin pages
- [x] 8.6 Write tests for both pages
...
## Tests
- Unit tests:
  - [x] SignupPage renders all form fields (org name, name, email, password, confirmation)
  - [x] SignupPage shows validation error when required fields are empty
  - [x] SignupPage shows error when passwords don't match
  - [x] SignupPage shows error when password is less than 8 characters
  - [x] SignupPage calls POST /api/signup with correct payload on valid submission
  - [x] SignupPage displays server error messages (e.g., duplicate email)
  - [x] SigninPage renders email and password fields
  - [x] SigninPage shows error for invalid credentials
  - [x] SigninPage calls signin function from AuthContext on submission
- Integration tests:
  - [x] SignupPage navigates to /signin on successful signup
  - [x] SigninPage navigates to / (dashboard) on successful signin
  - [x] SignupPage has link to /signin
  - [x] SigninPage has link to /signup
- Test coverage target: >=80% (`npm test` reported 91.31% statements, 83.13% branches, 97.1% functions, 91.31% lines)
- All tests must pass
